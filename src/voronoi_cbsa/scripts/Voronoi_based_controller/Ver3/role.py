...
class RoleManager():
    ...
    def TargetCallback(self, msg):
        """ 接收所有 Target 位置，並存入 self.target_positions """
        self.target_positions = []  # 清空舊的 Target 位置
        for target in msg.targets:
            target_x = target.position.x
            target_y = target.position.y
            self.target_positions.append(np.array([target_x, target_y]))

        # print(f"[Agent {self.id}] 收到目標位置: {self.target_positions}")




if __name__ == "__main__":
    print("[DEBUG] role.py 啟動！")
    rospy.loginfo("[DEBUG] role.py 啟動！")
